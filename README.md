...
> ⚠️ **تنبيه مهم عن هيكل الملفات**: يجب أن تكون هذه الملفات (`app/`, `package.json`, ...) في **جذر مستودع GitHub مباشرة** — وليست داخل مجلد فرعي. إن ظهر خطأ Vercel التالي:
> `Error: Couldn't find any 'pages' or 'app' directory`
> فهذا يعني أن الملفات تقع داخل مجلد وسيط (مثال: `repo/blood-bank-library/app/...`). الحل: تأكد أنك تنفّذ `git init` ثم `git add .` من **داخل هذا المجلد نفسه** الذي يحتوي `package.json` مباشرة، وليس من مجلد أبيه.
